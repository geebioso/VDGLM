...
% H = 0 means no significant autocorrelation 
% default to min(20, N-1) lags 
...
if (nargin >= 2) & ~isempty(Lags) 
   ...
   if any(round(Lags) - Lags) | any(Lags <= 0) 
      error(' All elements of ''Lags'' must be positive integers.') 
   end 
   if any(Lags > (n - 1)) 
      error(' All elements of ''Lags'' must not exceed ''Series'' length - 1.'); 
   end 
else 
...
if (nargin >= 3) & ~isempty(alpha) 
   ...
   if any(alpha <= 0 | alpha >= 1) 
      error(' All significance levels ''Alpha'' must be between 0 and 1.');  
   end 
   if length(alpha) == 1 
      alpha  =  alpha(ones(length(Lags),1));    % Scalar expansion. 
   end 
   if length(alpha) ~= length(Lags) 
      error(' Sizes of ''Alpha'' and ''Lags'' must be the same.'); 
   end 
else 
   alpha  =  0.05; 
end 
...
if (nargin >= 4) & ~isempty(DoF) 
   if prod(size(DoF)) ~= length(DoF)    % Check for a vector. 
      error(' ''DoF'' must be a vector.'); 
   end 
   DoF  =  DoF(:); 
   if any(round(DoF) - DoF) | any(DoF <= 0) 
      error(' All elements of ''DoF'' must be positive integers.') 
   end 
   if length(DoF) == 1 
      DoF  =  DoF(ones(length(Lags),1));    % Scalar expansion. 
   end 
   if length(DoF) ~= length(Lags) 
      error(' Sizes of ''DoF'' and ''Lags'' must be the same.'); 
   end 
   if any(DoF > Lags) 
      error(' No element of ''DoF'' may exceed corresponding element of ''Lags''.'); 
   end 
else 
   DoF  =  Lags; 
end 
...
maxLag =  max(Lags); 
N      =  [n - [1:maxLag]]'; 
ACF    =  autocorr_woolrich(Series , maxLag); 
ACF    =  ACF(2:end); 
